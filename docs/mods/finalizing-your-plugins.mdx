...
<Callout variant='info'>
If you support other recipe viewers that require synchronizing your mod's recipes (e.g. JEI), you can skip this step and synchronize your recipes via standard [Fabric](https://docs.fabricmc.net/develop/custom-recipe-types#recipe-synchronization)/[NeoForge](https://github.com/neoforged/NeoForge/pull/2021) APIs which will allow your mod to work with both recipe viewers as long as one is on the server.
</Callout>

For RRV to know how to synchronize your recipes from the server to the client, you have to synchronize your mod's recipes from the server to the client. This can be done at any point, but to guarantee things load at the right time you can do this in a serveride RRV plugin.
...
public class ExampleModClientIntegration implements ReliableRecipeViewerClientPlugin {
    ...
    public void onIntegrationInitialize() {
        ...
        ItemView.addClientRecipeProvider(recipeList -> { // provides a list of `ReliableClientRecipe`s to add to
            // Upgrading
            ClientRecipeManager.getRecipesForType(ExampleModRecipes.UPGRADING_RECIPE_TYPE).forEach(upgradingRecipeHolder -> { // provides a list of `RecipeHolder<UpgradingRecipe>`s for you to convert
