...
public class UpgradingClientRecipe implements ReliableClientRecipe {
    ...
    private final SlotContent baseItem, upgradeItem, output;
...
Slot that should not tick independently can be bound as a dependent slots in your client recipe. This can be used to keep recipe outputs that are dependent on the recipe inputs synchronized (e.g. dye recipes, decorated pot recipes).
