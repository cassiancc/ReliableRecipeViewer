...
public class CraftingClientRecipe implements ReliableClientRecipe {
    ...
    public void renderRecipe(RecipeViewScreen screen, RecipePosition recipePosition, GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (shapeless) {
            guiGraphics.blitSprite(RenderType::guiTextured, ResourceLocation.fromNamespaceAndPath("rrv", "crafting_shapeless"), 26, 14, 0, 0, 92, 0, 26, 14);
            if ((mouseX > 92 && mouseX < 122) && (mouseY>0 && mouseY < 14)) {
                guiGraphics.renderComponentTooltip(screen.getFont(), List.of(Component.translatable("view.rrv.type.crafting.shapeless")), mouseX+recipePosition.left(), mouseY+recipePosition.top());
            }
        }
